...
public class EBikeData implements Serializable{
	/**
	 ...
	 */
	public int message_notice_get;	
	/**
	 * @fields phone_call_get 电话呼叫标志接收完成
	 */
	public int phone_call_get;	
	/**
	 * @fields bat_connect 电池包连接标志
	 */
	public int bat_connect;	
	/**
	 * @fields ctrler_overcp 控制器过流保护
	 */
	public int ctrler_overcp;	
	/**
	 * @fields ctrler_lowvp 控制器欠压保护
	 */
	public int ctrler_lowvp;	
	/**
	 * @fields energy_cycle 能量回收状态
	 */
	public int energy_cycle;	
	/**
	 * @fields ctrler_err 控制器故障
	 */ 
	public int ctrler_err;	
	/**
	 * @fields back_led_on 后灯状态
	 */
	public int back_led_on;	
	/**
	 * @fields front_led_on 前灯状态
	 */
	public int front_led_on;
	/**
	 ...
	 */
	public int assis_mode;	
	
	/**
	 * @fields elec_mode 电动模式
	 */
	public int elec_mode;	
	
	
	/**
	 * @fields steppedfrequency 踏频量（圈）
	 */
	public int stepped_frequency;
	 ...
	 */
	public int battery_ah;
	
	/**
	 * @fields biking_state_change 骑行状态改变标志
	 */
	public int biking_state_change;
	
	/**
	 * @fields residual_capacity 剩余容量%
	 */
	public int residual_capacity;
	

	/**
	 * @fields mi_capacity 剩余里程
	 */
	public int mi_capacity;
	/**
...
//	public int cycle_times;
	 ...
	 * @Description 格式化数据
	 */
	public EBikeData setEBike(byte[] data){
		...
		if(bikeData.length>=10){
			stepped_frequency=ProtocolTool.byteArrayToInt(new byte[]{bikeData[0],bikeData[1]});
			biking_speed=ProtocolTool.byteArrayToInt(new byte[]{bikeData[2],bikeData[3]});
			accumulated_mileage=ProtocolTool.byteArrayToInt(new byte[]{bikeData[4],bikeData[5]});
			battery_ah=ProtocolTool.byteArrayToInt(new byte[]{bikeData[6]});
			biking_state_change=ProtocolTool.byteArrayToInt(new byte[]{bikeData[7]});
			residual_capacity=ProtocolTool.byteArrayToInt(new byte[]{bikeData[8]});
	...
	public String getValueText(){
		String value="短信提醒标志接收完成"+message_notice_get+
				"电话呼叫标志接收完成"+phone_call_get+
				"电池包连接标志"+bat_connect+
				"控制器过流保护"+ctrler_overcp+
				"控制器欠压保护"+ctrler_lowvp+
				"能量回收状态"+energy_cycle+
				"控制器故障"+ctrler_err+
				"后灯状态"+back_led_on+
				"前灯状态"+front_led_on+
				...
				"助力模式"+assis_mode+
				"电动模式"+elec_mode+
				"踏频量（圈）"+stepped_frequency+
				"骑行速度"+biking_speed+
				"累积骑行里程"+accumulated_mileage+
				"电池的安时数(100mah)"+battery_ah+
				"骑行状态改变标志"+biking_state_change+
				"剩余容量%"+residual_capacity+
				"剩余里程"+mi_capacity+
				"温度(℃)"+temperature+
//				"循环次数(次)"+cycle_times+
				"卡路里"+kcal_value;
