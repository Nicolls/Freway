package com.freway.ebike.upload;
    ...
    private void test(){
		...
		new Thread(){
			...
			public void run(){
				...
				// 文本参数
				multipartEntity.addStringPart("data", ebReq.getDataParam());
				multipartEntity.addStringPart("s", s);
...
//				multipartEntity.addBinaryPart("images", bitmapToBytes(bmp));
				// 文件参数
				multipartEntity.addFilePart("file", new File(photoPath));
				 
				// POST请求
				HttpPost post = new HttpPost(ebReq.getReqeustURL()) ;
				// 将multipartEntity设置给post
				post.setEntity(multipartEntity);
				// 使用http client来执行请求
				HttpClient httpClient = new DefaultHttpClient() ;
				try {
					System.out.println("发出去的："+EntityUtils.toString(post.getEntity(), "UTF-8"));
...
//    private final byte[] BIT_ENCODING = "Content-Transfer-Encoding: 8bit\r\n\r\n".getBytes();
     ...
     * 分隔符
     */
    private String mBoundary = "WebKitFormBoundarywONoFRmwyMpPvP9D";
    /**
     * 输出流
     */
...
//            mOutputStream.write(encodingBytes);
    ...
    public void addBinaryPart(String paramName, final byte[] rawData) {
        writeToOutputStream(paramName, rawData,  "", "no-file");
    }
    ...
    public void addFilePart(final String key, final File file) {
        ...
        try {
            ...
            writeFirstBoundary();
            final String type = CONTENT_TYPE + "image/png" + NEW_LINE_STR;
